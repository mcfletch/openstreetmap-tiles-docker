...
def coordinate(coord):
    try:
        coord = [float(x.strip()) for x in coord.strip('()[]').split(',')]
    except (ValueError, TypeError):
        raise argparse.ArgumentTypeError('Need a 4-coordinate point in -180.0,-90,180,90.0 format')
    if len(coord) != 4:
        raise argparse.ArgumentTypeError('Need exactly 4 coordinates for the bounding box')
    x, y, x1, y1 = coord 
    if x > x1:
        x1, x = x, x1 
    if y > y1:
    ...
    for coord in (x, x1):
        if coord < -180 or coord > 180:
            raise argparse.ArgumentTypeError('Longitude must be in range -180 to 180: %s'%(coord))
    for coord in (y, y1):
        if coord < -90 or coord > 90:
            raise argparse.ArgumentTypeError('Latitude must be in range -90 to 90: %s'%(coord))
    return [x, y, x1, y1]

parser = argparse.ArgumentParser(description='Render a map into static slippy-map images')
parser.add_argument(
    '--bbox', 
    dest = 'bbox', 
    metavar='X,Y,X1,Y1', 
    type=coordinate, 
    help='Bounding box in -180.0,-90,180,90.0 format', 
    default = (-180.0,-90,180,90.0), 
)
parser.add_argument(
    '-c','--config', metavar='FILE', dest='config',default='/usr/local/src/mapnik-style/osm.xml',
    help = 'The osm.xml file to be used for rendering', 
)
parser.add_argument(
    '-o','--output', metavar='DIR', dest='output',default='.',
    help = 'The osm.xml file to be used for rendering', 
)

parser.add_argument(
    '-z','--min-zoom', metavar='INT', dest='min_zoom', type=int, default=0, 
)
parser.add_argument(
    '-Z','--max-zoom', metavar='INT', dest='max_zoom', type=int, default=5, 
)
